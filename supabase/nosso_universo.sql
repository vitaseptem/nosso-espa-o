-- NOSSO UNIVERSO ❤️ — Banco de dados completo
-- Edite os e-mails na seção SEEDS antes de executar.
...
do $$ begin
  ...
  if not exists (select 1 from pg_type where typname = 'memory_category') then
    create type public.memory_category as enum ('foto', 'video', 'data', 'viagem', 'evento', 'outro');
  end if;
  if not exists (select 1 from pg_type where typname = 'event_type') then
    create type public.event_type as enum ('aniversario', 'casamento', 'namoro', 'viagem', 'compromisso', 'consulta', 'evento_familiar', 'outro');
  end if;
  if not exists (select 1 from pg_type where typname = 'mood_kind') then
    create type public.mood_kind as enum ('feliz', 'apaixonado', 'cansado', 'triste', 'irritado', 'animado', 'saudade');
  end if;
  ...
  if not exists (select 1 from pg_type where typname = 'call_status') then
    create type public.call_status as enum ('chamando', 'aceita', 'recusada', 'encerrada', 'perdida');
  end if;
  ...
  if not exists (select 1 from pg_type where typname = 'valentina_category') then
    create type public.valentina_category as enum ('primeiros_passos', 'primeiras_palavras', 'aniversarios', 'passeios', 'escola', 'momentos_especiais', 'conquistas', 'familia');
  end if;
  if not exists (select 1 from pg_type where typname = 'notification_type') then
    create type public.notification_type as enum ('mensagem', 'chamada', 'evento', 'lembrete', 'memoria', 'humor', 'pensamento', 'carta', 'sistema');
  end if;
end $$;
...
-- 5. FUNÇÃO is_authorized_user (após a tabela users existir)
create or replace function public.is_authorized_user()
returns boolean language plpgsql stable security definer
set search_path = public as $$
begin
  return exists (select 1 from public.users u where u.id = auth.uid());
end; $$;
...
-- 8. ADIVINHE MEU PENSAMENTO
create table if not exists public.thoughts_game (
  id            uuid primary key default gen_random_uuid(),
  author_id     uuid not null references public.users(id) on delete cascade,
  guesser_id    uuid references public.users(id) on delete set null,
  hint          text,
...
do $$ declare t text; tables text[] := array['app_settings','users','memories','events','valentina_memories'];
begin
...
begin
  ...
  if user_count > 2 then
    raise exception 'NOSSO UNIVERSO: limite de 2 usuários atingido.';
  end if;
  select * into allowed from public.app_allowed_users where email = new.email;
  if not found then
    raise exception 'NOSSO UNIVERSO: e-mail % não autorizado.', new.email;
  end if;
  insert into public.users (id, email, role, display_name)
  values (new.id, new.email, allowed.role, allowed.display_name)
  on conflict (id) do update set email = excluded.email, role = excluded.role, display_name = excluded.display_name;
  return new;
end; $$;
...
begin
  if (select count(*) from public.users) >= 2 then
    raise exception 'NOSSO UNIVERSO: já existem 2 perfis.';
  end if;
  return new;
end; $$;
...
create policy allowed_users_no_access on public.app_allowed_users for all using (false) with check (false);

drop policy if exists settings_select on public.app_settings;
create policy settings_select on public.app_settings for select using (public.is_authorized_user());
drop policy if exists settings_update on public.app_settings;
create policy settings_update on public.app_settings for update using (public.is_authorized_user()) with check (public.is_authorized_user());

drop policy if exists users_select on public.users;
create policy users_select on public.users for select using (public.is_authorized_user());
...
create policy memories_all on public.memories for all using (public.is_authorized_user()) with check (public.is_authorized_user() and author_id = auth.uid());

drop policy if exists memory_media_all on public.memory_media;
create policy memory_media_all on public.memory_media for all using (public.is_authorized_user()) with check (public.is_authorized_user());

drop policy if exists events_all on public.events;
create policy events_all on public.events for all using (public.is_authorized_user()) with check (public.is_authorized_user() and created_by = auth.uid());

drop policy if exists thoughts_select on public.thoughts_game;
create policy thoughts_select on public.thoughts_game for select using (public.is_authorized_user());
drop policy if exists thoughts_insert on public.thoughts_game;
create policy thoughts_insert on public.thoughts_game for insert with check (public.is_authorized_user() and author_id = auth.uid());
drop policy if exists thoughts_update on public.thoughts_game;
create policy thoughts_update on public.thoughts_game for update using (public.is_authorized_user()) with check (public.is_authorized_user());
drop policy if exists thoughts_delete on public.thoughts_game;
create policy thoughts_delete on public.thoughts_game for delete using (author_id = auth.uid());

drop policy if exists mood_select on public.mood_logs;
create policy mood_select on public.mood_logs for select using (public.is_authorized_user());
drop policy if exists mood_modify on public.mood_logs;
create policy mood_modify on public.mood_logs for all using (user_id = auth.uid()) with check (user_id = auth.uid());

drop policy if exists messages_select on public.messages;
create policy messages_select on public.messages for select using (public.is_authorized_user());
drop policy if exists messages_insert on public.messages;
create policy messages_insert on public.messages for insert with check (public.is_authorized_user() and sender_id = auth.uid());
drop policy if exists messages_update on public.messages;
create policy messages_update on public.messages for update using (public.is_authorized_user()) with check (public.is_authorized_user());
...
create policy calls_select on public.audio_calls for select using (caller_id = auth.uid() or callee_id = auth.uid());
drop policy if exists calls_insert on public.audio_calls;
create policy calls_insert on public.audio_calls for insert with check (caller_id = auth.uid());
drop policy if exists calls_update on public.audio_calls;
create policy calls_update on public.audio_calls for update using (caller_id = auth.uid() or callee_id = auth.uid()) with check (caller_id = auth.uid() or callee_id = auth.uid());

drop policy if exists signals_select on public.call_signals;
create policy signals_select on public.call_signals for select using (
  exists (select 1 from public.audio_calls c where c.id = call_id and (c.caller_id = auth.uid() or c.callee_id = auth.uid()))
);
drop policy if exists signals_insert on public.call_signals;
create policy signals_insert on public.call_signals for insert with check (
  sender_id = auth.uid()
  and exists (select 1 from public.audio_calls c where c.id = call_id and (c.caller_id = auth.uid() or c.callee_id = auth.uid()))
);

drop policy if exists letters_select on public.future_letters;
create policy letters_select on public.future_letters for select using (
  public.is_authorized_user() and (author_id = auth.uid() or open_date <= current_date)
);
drop policy if exists letters_insert on public.future_letters;
create policy letters_insert on public.future_letters for insert with check (public.is_authorized_user() and author_id = auth.uid());
drop policy if exists letters_update on public.future_letters;
create policy letters_update on public.future_letters for update using (author_id = auth.uid() or open_date <= current_date) with check (public.is_authorized_user());
drop policy if exists letters_delete on public.future_letters;
create policy letters_delete on public.future_letters for delete using (author_id = auth.uid());

drop policy if exists val_memories_all on public.valentina_memories;
create policy val_memories_all on public.valentina_memories for all using (public.is_authorized_user()) with check (public.is_authorized_user() and author_id = auth.uid());

drop policy if exists val_media_all on public.valentina_media;
create policy val_media_all on public.valentina_media for all using (public.is_authorized_user()) with check (public.is_authorized_user());

drop policy if exists notifications_select on public.notifications;
create policy notifications_select on public.notifications for select using (user_id = auth.uid());
drop policy if exists notifications_insert on public.notifications;
create policy notifications_insert on public.notifications for insert with check (public.is_authorized_user());
...
-- 20. REALTIME
do $$ declare t text; rt_tables text[] := array['messages','audio_calls','call_signals','notifications','thoughts_game','mood_logs','users'];
begin
...
-- 21. SEEDS — EDITE OS E-MAILS ANTES DE RODAR
insert into public.app_allowed_users (email, role, display_name) values
  ('marido@email.com', 'marido', 'Marido'),
  ('esposa@email.com', 'esposa', 'Esposa')
on conflict (email) do update set role = excluded.role, display_name = excluded.display_name;

insert into public.app_settings (id, couple_name, valentina_name)
values (1, 'Nosso Universo', 'Valentina')
